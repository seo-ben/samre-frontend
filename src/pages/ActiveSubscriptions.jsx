...
import { Loader2, AlertCircle, CheckCircle2, X, XCircle, Search, User, CreditCard, Calendar } from 'lucide-react';
...
export const ActiveSubscriptionsPage = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const res = await apiClient.get('/v1/admin/user-subscriptions?status=active');
      setSubscriptions(res.data.data || res.data || []);
      setError(null);
    } catch (err) {
      console.error(err);
      setError("Erreur lors du chargement des abonnés actifs.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCancel = async (id) => {
    ...
    try {
      ...
      fetchData(); // Rafraîchir la liste (l'abonnement n'apparaîtra plus s'il est filtré par status=active)
    } catch (err) {
      ...
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: '#8A94A6', fontSize: '14px' }}>
            Consultez et gérez les abonnements actuellement en cours de validité.
          </p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, textAlign: 'left', fontSize: '14px' }}>
            <thead>
              <tr>
                ...
                <th style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#FAFAFA', borderBottom: '1px solid #E4E4E7', width: '20%', height: '40px', padding: '0 16px', fontWeight: '500', color: '#71717A', verticalAlign: 'middle' }}>Plan Actuel</th>
                <th style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#FAFAFA', borderBottom: '1px solid #E4E4E7', width: '15%', height: '40px', padding: '0 16px', fontWeight: '500', color: '#71717A', verticalAlign: 'middle' }}>Dates (Début - Fin)</th>
                <th style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#FAFAFA', borderBottom: '1px solid #E4E4E7', width: '15%', height: '40px', padding: '0 16px', fontWeight: '500', color: '#71717A', verticalAlign: 'middle' }}>Renouvellement</th>
