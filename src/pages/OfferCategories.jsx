...
export const OfferCategoriesPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await apiClient.put(`/v1/admin/cms/dynamic/job-categories/${editingItem.id}`, payload);
        showToast("Catégorie mise à jour avec succès");
      } else {
        await apiClient.post(`/v1/admin/cms/dynamic/job-categories`, payload);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/v1/admin/cms/dynamic/job-categories/${deleteConfirm.id}`);
  ...
  return (
    <MainLayout>
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: '#8A94A6', fontSize: '14px' }}>
            Gérez les secteurs d'activité et les métiers (catégories principales et sous-catégories).
          </p>
        </div>
        <button
              ...
              {categories.length === 0 ? (
                <tr>
                  <td colSpan="6" style={{ padding: '40px', textAlign: 'center', color: '#6B7280' }}>
                    Aucune catégorie d'offre trouvée.
                  </td>
                </tr>
              ) : categories.map((item) => {
      ...
      <ConfirmModal
        ...
        title="Supprimer la catégorie d'offre"
